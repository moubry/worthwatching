...
package com.moubry.worthwatching.ui.widget;
...
public class DashboardLayout extends ViewGroup {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if(rows * cols > visibleCount + 1)
        {
        	if(cols > rows)
        	{
        		// Make sure we don't increase the cols.
        		if ((visibleCount % rows == 0) && (visibleCount / rows <= cols))
        		{
        			//they can be split evenly among rows
            		cols = visibleCount / rows;
            		 hSpace = ((width - mMaxChildWidth * cols) / (cols + 1));
        	...
        	{
        		// Make sure we don't increase the rows.
        		if ((visibleCount % cols == 0) && (visibleCount / cols <= rows))
        		{
        			//they can be split evenly among cols
            		rows = visibleCount / cols;
                     vSpace = ((height - mMaxChildHeight * rows) / (rows + 1));
